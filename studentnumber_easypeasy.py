...
def calculate_student_counts(city, region, housing_type, subtype, scale, units, tab):
    ...
    e = round(units * e_rate / 100, 1)
    m = round(units * m_rate / 100, 1)
    h = round(units * high_school_personnel * high_school_rate / 100, 1)
    h_basis = f"세대수 X {high_school_personnel}명 X {high_school_rate:.1f}%"

    result_table = f"""
| 주택유형         | 공급유형 | 주택규모 | 세대수 | 초등학생 | 중학생 | 고등학생 |
| :-------------: | :------: | :------: | :----: | :------: | :----: | :------: |
| {housing_type} | {subtype} | {scale}  | {units}  | {e}명    | {m}명    | {h}명    |
"""
    tab.success("✅ 예상 학생 수:\n" + result_table)

    calculation_basis = f"""
📝 **추정 학생 수 계산 근거**:
- **초등학생**: 세대수 X 초등학생 발생률 {e_rate}%({region} 발생률 적용)
- **중학생**: 세대수 X 중학생 발생률 {m_rate}%({region} 발생률 적용)
- **고등학생**: {h_basis}
"""
...
def process_excel(file, tab):
    try:
        ...
        if not required_cols.issubset(df.columns):
            tab.error(
                "❌ 엑셀에 필수 열(시, 지역, 주택유형, 공급유형, 주택규모, 초등, 중등, 고등학생 인원, 고등학생 발생률)이 부족합니다."
            )
            return
        ...
        for _, row in df.iterrows():
            ...
            e, m = round(float(row["초등"]), 1), round(float(row["중등"]), 1)
            new_rates.setdefault(full_region, {})
            new_rates[full_region].setdefault(h, {})
            new_rates[full_region][h].setdefault(s, {})[sc] = {"초등": e, "중등": m}

            high_school_personnel = row.get("고등학생 인원")
            high_school_rate = row.get("고등학생 발생률")
            ...
            if pd.notna(high_school_personnel) and pd.notna(high_school_rate) and high_school_personnel != "" and high_school_rate != "":
                ...
                new_high_school_rates[full_region][h].setdefault(s, {})[sc] = {
                    "인원": round(float(high_school_personnel), 1),
                    "발생률": round(float(high_school_rate), 1),
                }

        # JSON 파일 저장
        save_json(YIELD_RATE_FILE, new_rates)
        save_json(HIGH_SCHOOL_YIELD_RATE_FILE, new_high_school_rates)

        # 세션 상태 업데이트
        st.session_state.rates = load_json(YIELD_RATE_FILE)
        st.session_state.high_school_rates = load_json(HIGH_SCHOOL_YIELD_RATE_FILE)

        tab.success(f"✅ {len(df)}개 조합 저장 및 업데이트 완료!")
...
def to_excel(df):
    ...
    try:
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            ...
            cols_to_format = ["초등", "중등", "고등학생 인원", "고등학생 발생률"]
            for col_num, col_name in enumerate(df.columns):
                if col_name in cols_to_format:
                    column_letter = chr(ord("C") + col_num - 1)
...
def main():
    ...
    if "calculated_results" not in st.session_state or not st.session_state.calculated_results:
        ...
        result_df = result_df[
            ["시", "지역", "주택유형", "공급유형", "주택규모", "세대수", "초등학생", "중학생", "고등학생"]
        ]
        result_tab.dataframe(result_df, use_container_width=True, hide_index=True)

        options = [f"{i+1} 번째 줄" for i in range(len(st.session_state.calculated_results))]
        selected_indices = result_tab.multiselect("삭제할 계산 결과 선택", options)
        if result_tab.button("🗑️ 선택한 결과 삭제"):
            original_indices = [i for i, option in enumerate(options) if option in selected_indices]
            new_results = [
                result
...
""",
    ...
    rate_tab.markdown(
        '<div style="text-align: right; font-size: 0.8em;">(단위: %)</div>',
        unsafe_allow_html=True,
    )
    ...
    rate_data = [
        {
            ...
            "초등": v.get("초등", 0.0),
            "중등": v.get("중등", 0.0),
            "고등학생 인원": st.session_state.high_school_rates.get(r, {})
            .get(t, {})
            .get(s, {})
            .get(sc, {})
            .get("인원", 0.0)
            ...
            "고등학생 발생률": st.session_state.high_school_rates.get(r, {})
            .get(t, {})
            .get(s, {})
            .get(sc, {})
            .get("발생률", 0.0)
