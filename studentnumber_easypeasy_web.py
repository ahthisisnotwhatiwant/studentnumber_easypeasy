...
def main():
    ...
    intro_tab.markdown(
        ...
        💁 **사용 순서 및 방법**
        1.  (처음 사용 시) 4번째 탭 하단에 위치한 '기존 정보 다운로드'를 눌러 다운받아주세요.
        2.  (처음 사용 시) 다운받은 '학생 발생률 정보' 엑셀 파일을 더블클릭하여 열어주세요.
        3.  (처음 사용 시) 학생 발생률 정보를 엑셀 파일 내 서식에 맞추어 작성 후 저장해주세요.
        4.  4번째 탭 중간에 위치한 'Browse files'를 눌러 최신 '학생 발생률 정보' 엑셀 파일을 업로드해주세요.
        5.  2번째 탭으로 이동하여 계산해주세요.

        ⚠️ **주의 및 참고 사항**
        - 앱 종료 후 재실행 시 기존에 업로드한 학생 발생률 정보가 삭제되고 초기 예시로 돌아갑니다.
        - 반드시 작성 후 저장한 최신 '학생 발생률 정보' 엑셀 파일을 삭제하지 말고 보관해주세요.
        - 업로드한 '학생 발생률 정보' 엑셀 파일의 정보는 서버에 남지 않으며 타 이용자가 알 수 없습니다.
        - '학생 발생률 정보' 엑셀 파일의 정보가 외부로 유출되지 않게 유의해주세요.
    ...
    rate_tab.info("①기존 정보 다운로드 ②엑셀 내 정보 작성 및 수정 ③엑셀 업로드 → 자동 연동")
